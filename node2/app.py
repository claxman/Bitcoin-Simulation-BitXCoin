...
class Blockchain:
    ...
    def hash(self, block):
        # Ensuring the dictionary is ordered
...
def new_transaction():
    values = request.form
    transaction_results = blockchain.submit_transaction(values['sender_public_key'], values['recipient_public_key'], values['signature'], values['amount'])

    

    if transaction_results == False:
        url = values['url']
        url = url+"retryNew/transaction"
        return redirect(url)
    else:
        url = values['url']
